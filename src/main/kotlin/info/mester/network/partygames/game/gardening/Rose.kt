...
class Rose(
    ...
    override fun spawn() {
        when (level) {
            0 -> placeBlock(Vector(0.0, 0.0, 0.0), Material.POPPY)
            1 -> {
                val lower = placeBlock(Vector(0.0, 0.0, 0.0), Material.ROSE_BUSH)
                val upper = placeBlock(Vector(0.0, 1.0, 0.0), Material.ROSE_BUSH)
